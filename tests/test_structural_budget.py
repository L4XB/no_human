...
FROZEN_FILE_LINES = {
    ...
    # 9048 -> 9059 (+11): #343 (PR #379) gives task `config` its own writer,
    # so the three `apply_action` call sites in this file each persist the
    # raise through `update_task_config` beside the action that produced it,
    # instead of letting a generic save write a stale blob back. Written
    # there deliberately rather than inside a shared helper, so the config
    # write is visible at the point a human's override is applied.
    # Measured on the merge result with the scanner below.
    ...
    # 6340 -> 6346 (+6): #343 (PR #379) persists a blocker-action config
    # change through `update_task_config` at the API call site, for the same
    # reason as `cli/commands.py` above — a human's budget raise must not be
    # written back by whoever holds a stale task handle.
    # Measured on the merge result with the scanner below.
    "api/app.py": 6346,
    ...
    # `update_task` and `update_task_columns` why `config` is now excluded —
    # the same shape `set_status` already has for `status`. Most of the +37 is
    # that reasoning: the next person adding a column to those writers needs to
    # know `config` is deliberately absent rather than forgotten.
    # Measured on the merge result with the scanner below.
    "core/db.py": 5322,
